...
exports.createMovie = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "Movie created successfully",data:saved});
...
exports.getMovieById = async (req, res) =>{
    try {
        ...
        if(!movie) return res.status(404).json({ message: "Movie not found" }); 
            ...
            res.status(200).json({message:"Movie get successfully",data:movie});
            } catch (err) {
                res.status(400).json({ message: err.message });
                }
}
...
exports.updateMovie = async (req, res) => {
    try {
      ...
      res.json({message:"Movie Updated succesfully",data:movie});
    } catch (err) {
